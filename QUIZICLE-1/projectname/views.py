...
class QuizCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['user_quizzes'] = Quiz.objects.filter(creator=self.request.user)
        return context
    ...
    def _save_questions_and_answers(self, quiz):
        ...
        for index, question_text in enumerate(questions):
            ...
            # Get the question image and description image by index
            question_image = question_images[index] if index < len(question_images) else None
            description_image = description_images[index] if index < len(description_images) else None
...
class ModifyQuizView(View):
    ...
    def post(self, request, quiz_id):
        ...
        # Remove extra questions if user shortened the list
        if len(existing_questions) > len(new_questions):
            for q in existing_questions[len(new_questions):]:
                q.delete()
...
@login_required
def quiz_data_api(request, quiz_id):
    quiz = get_object_or_404(Quiz, id=quiz_id, creator=request.user)
    ...
    for q in quiz.questions.all():
        question_data = {
            ...
            'points_for_question': q.points_for_question,
            'image_url': q.image.url if q.image else '',
            'description_text': q.answer_description.text if hasattr(q, 'answer_description') else '',
            'description_image_url': q.answer_description.image.url if (hasattr(q, 'answer_description') and q.answer_description.image) else '',
            'answers': []
        }
